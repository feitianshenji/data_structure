...
class SingleILinkedList<T> implements ILinkedList<T> {

    protected Node<T> head;//带数据头结点

    public SingleILinkedList(Node<T> head) {//构造函数
        this.head = head;
    }
    public SingleILinkedList() {//构造函数
     ...
     * @param array
     */
    public SingleILinkedList(T[] array) {//构造函数
        this.head = null;
    ...
    //根据下标添加结点:1.头部插入2.中间插入3.末尾插入.下标值从0开始
    public boolean add(int index, T data) {
        ...
        if (this.head==null || index<=1) {
            ...
            //尾部添加和中间插入属于同种情况，毕竟当front为尾部结点时，front.next=null
            p.next = front.next;
            front.next = p;
        }
        return true;
...
public class SingleILinkedListTest {
