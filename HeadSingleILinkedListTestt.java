...
interface ILinkedList<T> {
     ...
     * @param list
     */
    void concat(HeadSingleILinkedList<T> list);
    /**
     * 输出格式
     * @return
     */
 ...
 *  单项链表的实现,带头结点并含有尾指针的链表
 */
class HeadSingleILinkedList<T> implements ILinkedList<T> {

    protected Node<T> headNode;//不带数据头结点
    protected Node<T> rear;//指向尾部的指针

    public HeadSingleILinkedList() {//构造函数
        //初始化头结点与尾指针
        this.headNode = rear = new Node<>(null);
    }

    public HeadSingleILinkedList(Node<T> head) {//构造函数
        this();
    ...
    //根据下标添加结点,1.头部插入2.中间插入3.末尾插入.该值从0开始,如传4就代表插入在第5个位置
...
public class HeadSingleILinkedListTestt {
